...
def pymunk_to_shapely(body, shapes):
    """Convert a pymunk body to a shapely geometry."""
    geoms = []
    for shape in shapes:
...
class PushTEnv(gym.Env):
    """
    ## Description of the PushT environment.
    ...
    * TODO:
    ...
    def __init__(
        ...
        self.action_space = spaces.Box(
            low=0, high=512, shape=(2,), dtype=np.float32
        )
    ...
    def reset(self, seed=None, options=None, target_coordinates=None):
        ...
        if target_coordinates is not None:
            observation, info = self.get_image_target(
                target_coordinates, noise=True
            )
    ...
    def _draw(self):
        ...
        for shape in self.block.shapes:
            ...
            pygame.draw.polygon(
                screen, pygame.Color("LightGreen"), goal_points
            )
    ...
    def get_obs(self):
        if self.obs_type == "state":
            ...
            return np.concatenate(
                [agent_position, block_position, [block_angle]],
                dtype=np.float64,
            )

        if self.obs_type == "environment_state_agent_pos":
            return {
                "environment_state": self.get_keypoints(
                    self._block_shapes
                ).flatten(),
    ...
    def _setup(self):
        ...
        self.block, self._block_shapes = self.add_tee(
    ...
    @staticmethod
    def add_tee(
        space, position, angle, scale=30, color="LightSlateGray", mask=None
    ):
        if mask is None:
    ...
    def get_image_target(self, target_pose, noise=False):
        super().reset()
        self.space = pymunk.Space()
        self.space.gravity = 0, 0
        self.space.damping = self.damping if self.damping is not None else 0.0
        ...
        self.goal_pose = target_pose  # x, y, theta (in radians)
        if self.block_cog is not None:
            self.block.center_of_gravity = self.block_cog
        ...
        if noise:
            state[2:4] += np.random.normal(0, 5, 2)
        self._set_state(state)
        observation = self.get_obs()
        info = self._get_info()
        info["is_success"] = False
